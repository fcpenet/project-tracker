...
export default function NewEpicModal({ projectId, onCreated, onClose }: Props) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setLoading(true)
    try {
      await epicService.create(projectId, title.trim(), description.trim() || undefined)
      onCreated()
    } catch (err: unknown) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div className="bg-[#13151d] border border-[#2a2d36] rounded-lg w-full max-w-md mx-4 p-6">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-sm font-semibold text-gray-200">New Epic</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1">
            <label htmlFor="epic-title" className="text-xs text-gray-400">Title <span className="text-red-400">*</span></label>
            <input
              id="epic-title"
              type="text"
              value={title}
          ...
          <div className="flex flex-col gap-1">
            <label htmlFor="epic-desc" className="text-xs text-gray-400">Description</label>
            <textarea
              id="epic-desc"
              value={description}
